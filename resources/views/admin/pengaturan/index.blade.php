            ...
            <div class="{{ $item->tipe === 'textarea' || $item->tipe === 'image' ? 'md:col-span-2' : '' }}">
                ...
                @elseif($item->tipe === 'textarea')
                    <textarea name="{{ $item->kunci }}" 
                              id="{{ $item->kunci }}"
                              rows="4"
                              class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">{{ old($item->kunci, $item->nilai) }}</textarea>
